...
class SuperSAB(object):
    def __init__(self, delta0=0.01, alpha=1, delta_max=50, delta_min=1e-6, epoch_max=10000, Ni=2, Nh=15, Ns=1):
        self.delta0 = delta0
        self.alpha = alpha
            ...
            if epoca == 0:
                ...
                for i in xrange(sizekj[0]):
                    for j in xrange(sizekj[1]):
                        if (Dkj[i,j] > 0):
                            self.taWkj[i,j] = min(self.taWkj[i,j] * self.eta_plus, self.delta_max)#self.taWkj[i,j] * self.eta_plus#min(self.taWkj[i,j] * self.eta_plus, self.delta_max)
                        elif (Dkj[i,j] < 0):
                            self.taWkj[i,j] = max(self.taWkj[i,j] * self.eta_less, self.delta_min)#self.taWkj[i,j] * self.eta_less#max(self.taWkj[i,j] * self.eta_less, self.delta_min)

                self.Wkj = (-self.taWkj * gradkj) + (self.alpha * self.Wkj)
                self.Wji = (-self.taWji * gradji) + (self.alpha * self.Wji)
